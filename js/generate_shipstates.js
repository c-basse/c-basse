...
function generate_shipstates(ship_config, options) {
	...
	if (ship_config.upgrades.supernatural_reflexes){
		shipstateArray = action_phase(shipstateArray, ship_config, {require_move: false, slam_allowed: false});
	}
	...
	//Execute maneuver
	shipstateArray = maneuver_phase(shipstateArray, ship_config.move_sets.maneuver_set, ship_config);
...
function maneuver_phase(shipstateArray, move_set, ship_config,
	{intermediate_move = false, must_if_unstressed = false}={}) {
	new_shipstateArray = [];
...
function action_phase(shipstateArray, ship_config,
	{require_move = true, disable_future_actions = false, boost_allowed = true, roll_allowed = true, slam_allowed = true, force_red =  false}={}) {
	new_shipstateArray = [];
	for (var i=0; i<shipstateArray.length; i++) {
				...
				switch(shipstateArray[i].actions_remaining[j]) {
					case "Barrel Roll":
						for (var move_name in ship_config.move_sets.roll_set) {
							...
							for(var k=0; k<loop_count; k++){
								...
								if (maneuver.enabled && roll_allowed) {                //check if this roll is enabled
									var ship = shipstateArray[i].clone();
									ship.add_move(maneuver);
									if(disable_future_actions){
										ship.actions_disabled = true;
									}
									...
									new_shipstateArray.push(ship);
									ship.actions_remaining.splice(j,1);
									if(ship_config.ship_ability.autothrusters){
										new_shipstateArray = new_shipstateArray.concat(action_phase([ship], ship_config, {require_move: false, disable_future_actions: false, slam_allowed: false, force_red: true}));
									} else if (ship_config.ship_ability.refined_gyrostabilizers) {
										new_shipstateArray = new_shipstateArray.concat(action_phase([ship], ship_config, {require_move: false, disable_future_actions: false, slam_allowed: false, roll_allowed: false, force_red: true}));
					...
					case "Boost":
						for (var move_name in ship_config.move_sets.boost_set) {
							if (ship_config.move_sets.boost_set[move_name].enabled && boost_allowed) {                //check if this boost is enabled
								var ship = shipstateArray[i].clone();
								ship.add_move(ship_config.move_sets.boost_set[move_name]);
								if(disable_future_actions){
									ship.actions_disabled = true;
								}
						...
						break;
					case "Slam":
						//(need to recheck stress becuase primed thrusters could let something through
						cdf = 2; //not implemented yet
						break;
				}
			}
		}
